...
- type: image
  options:
    imagePath: https://i.ibb.co/yhbt6CY/dashworks.png
```
...
Note, config for this widget is case-sensetive (see [#1268](https://github.com/KhulnaSoft/dashworks/issues/1268))
...
**`repos`** | `array` |  _Optional_ | If you'd like to also display stats for some GitHub repositories, then add an array or repo names here. Specified as `[username]/[repo-name]`, e.g. `khulnasoft/dashworks`
...
This widget allows searching multiple search engines from dashworks.
```yaml
  - type: 'custom-search'
    options:
...
**`user_name`** | `string` |  Required | A Proxmox API Username, for example `root@pam` or `dashworks@pve`.
...
- type: embed
  options:
    html: '<img src="https://dashworks.lan/item-icons/my-image.png" />'
```
...
For a full tutorial on creating your own widget, you can follow [this guide](/docs/development-guides.md/#building-a-widget), or take a look at [here](https://github.com/KhulnaSoft/dashworks/commit/3da76ce2999f57f76a97454c0276301e39957b8e) for a code example.
