...
1. Pull latest image: `docker pull khulnasoft/dashworks:latest`
2. Kill off existing container
	- Find container ID: `docker ps`
	- Stop container: `docker stop [container_id]`
	- Remove container: `docker rm [container_id]`
3. Spin up new container: `docker run [params] khulnasoft/dashworks`
...
- Finally, to save the backup locally, run `docker save -o ~/dashworks-backup.tar my-backup`
...
It's helpful to be aware of any potential security issues in any of the Docker images you are using. You can run a quick scan using Snyk on any image to output known vulnerabilities using [Docker scan](https://docs.docker.com/engine/scan/), e.g: `docker scan khulnasoft/dashworks:latest`.

A similar product is [Trivy](https://github.com/aquasecurity/trivy), which is free an open source. First install it (with your package manager), then to scan an image, just run: `trivy image khulnasoft/dashworks:latest`
...
Create a new file in `/etc/nginx/sites-enabled/dashworks`
...
Add a `.htaccess` file within `/var/www/html/dashworks/.htaccess`, and add:
...
You may wish to upload your image to a container registry for easier access. Note that if you choose to do this on a public registry, please name your container something other than just 'dashworks', to avoid confusion with the official image.
